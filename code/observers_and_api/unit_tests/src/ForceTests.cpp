...
DiffractionForceModel ForceTests::get_diffraction_force_model(const YamlModel& waves, const std::string& diffraction_yaml, const std::string& hdb_file_contents, EnvironmentAndFrames& env) const
{
    ...
    env = sys.get_env();
    const ssc::kinematics::Transform T(ssc::kinematics::Point("NED",0,0,0), body.name);
    env.k->add(T);
...
BodyStates get_whole_body_state_with_psi_equal_to(const double psi, const std::string& body_name)
{
    ...
    states.G = ssc::kinematics::Point(body_name,0,0,0);
    return states;
}
...
// TODO: find out why this test generates a segfault (when calling env.k->get() in DiffractionForceModel)
TEST_F(ForceTests, bug_3210_no_interpolation_in_incidence_and_no_incidence_no_interpolation_in_period_no_transport)
{
    ...
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_4, config_such_that_rao_point_is_zero, test_data::bug_3210(), env);
    ssc::data_source::DataSource ds;
    const auto states = get_whole_body_state_with_psi_equal_to(0, F.get_body_name());
    const double t = 0;
    const auto tau = F(states, t, env, ds);
...
TEST_F(ForceTests, bug_3210_no_interpolation_in_incidence_interpolation_in_period_no_transport)
{
    ...
    const std::string config_such_that_rao_point_is_zero = get_diffraction_conf(0,0,0);
    EnvironmentAndFrames env;
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_10, config_such_that_rao_point_is_zero, test_data::bug_3210(), env);
    const auto states = get_whole_body_state_with_psi_equal_to(0, F.get_body_name());
    const double t = 0;
    ssc::data_source::DataSource ds;
    const auto tau = F(states, t, env, ds);
...
TEST_F(ForceTests, bug_3210_no_interpolation_in_incidence_no_interpolation_in_period_transport)
{
    ...
    EnvironmentAndFrames env;
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_4, config_such_that_rao_point_is_not_zero, test_data::bug_3210(), env);
    const auto states = get_whole_body_state_with_psi_equal_to(0, F.get_body_name());
    const double t = 0;
    ssc::data_source::DataSource ds;
    const auto tau = F(states, t, env, ds);
...
TEST_F(ForceTests, bug_3210_no_interpolation_in_incidence_interpolation_in_period_transport_with_non_zero_t)
{
    ...
    const std::string config_such_that_rao_point_is_not_zero = get_diffraction_conf(1,2,3);
    EnvironmentAndFrames env;
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_10, config_such_that_rao_point_is_not_zero, test_data::bug_3210(), env);
    const auto states = get_whole_body_state_with_psi_equal_to(0, F.get_body_name());
    const double t = 3;
    ssc::data_source::DataSource ds;
    const auto tau = F(states, t, env, ds);
...
TEST_F(ForceTests, bug_3210_interpolation_with_non_zero_psi_in_incidence_no_interpolation_in_period_no_transport)
{
    ...
    EnvironmentAndFrames env;
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_4, config_such_that_rao_point_is_zero, test_data::bug_3210(), env);
    const auto states = get_whole_body_state_with_psi_equal_to(12, F.get_body_name());
    const double t = 0;
    ssc::data_source::DataSource ds;
    const auto tau = F(states, t, env, ds);
    // Interpolate at 15° using the following lines of the HDB file (see two previous tests):
...
TEST_F(ForceTests, bug_3239_reference_frame_is_incorrect)
{
    ...
    EnvironmentAndFrames env;
    DiffractionForceModel F = get_diffraction_force_model(regular_waves_Hs_1_propagating_to_north_Tp_equals_4, config_such_that_rao_point_is_zero, test_data::bug_3210(), env);
    const auto states = get_whole_body_state(0,0,0,0,0,0, F.get_body_name());
    const double t = 0;
    ssc::data_source::DataSource ds;
    const auto tau = F(states, t, env, ds);
    ASSERT_EQ("TestShip", tau.get_frame());
}
