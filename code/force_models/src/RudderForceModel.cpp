...
RudderForceModel::RudderForceModel(const Yaml& input_, const std::string& body_name, const EnvironmentAndFrames& env) :
        ForceModel(input_.name, body_name, env, input_.position_of_propeller_frame, {"rpm","P/D","beta"}),
        propulsion(WageningenControlledForceModel(input_, body_name , env)),
