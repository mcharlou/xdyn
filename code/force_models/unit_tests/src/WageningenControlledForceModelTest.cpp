...
TEST_F(WageningenControlledForceModelTest, torque_should_have_sign_corresponding_to_rotation)
{
    ...
    const EnvironmentAndFrames env = get_env();
    //env.rho = a.random<double>().greater_than(0); // Is this necessary ?? From get_env(), rho is set to 1024.
...
TEST_F(WageningenControlledForceModelTest, bug_2825_can_use_propeller_with_rpm_near_zero)
{
    ...
    auto force = w.get_force(states, a.random<double>(), env, commands);

    ASSERT_NEAR(0, force(0), EPS);
    ASSERT_NEAR(0, force(1), EPS);
    ASSERT_NEAR(0, force(2), EPS);
    ASSERT_NEAR(0, force(3), EPS);
    ASSERT_NEAR(0, force(4), EPS);
    ASSERT_NEAR(0, force(5), EPS);
}
