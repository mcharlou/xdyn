...
//! [ResistanceCurveForceModelTest expected output]
    ...
    states.u.record(0, 0);
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).X());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Y());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Z());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).K());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).M());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).N());

    states.u.record(1, 0.514444444444444);
    ASSERT_NEAR(-1e6, (double)F(states, a.random<double>(), env,  ds).X(), EPS);
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Y());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Z());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).K());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).M());
    ...
    ASSERT_NEAR(-225e6, (double)F(states, a.random<double>(), env,  ds).X(), EPS);
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Y());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).Z());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).K());
    ASSERT_DOUBLE_EQ(0, (double)F(states, a.random<double>(), env,  ds).M());
...
TEST_F(ResistanceCurveForceModelTest, should_issue_a_warning_when_speed_is_lower_than_min_speed_specified_in_resistance_curve_table)
{
    ...
    states.u.record(0, -1);
    F(states, a.random<double>(), env, ds);
